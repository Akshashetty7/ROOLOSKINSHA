...
router.route("/auth/register")
  ...
  .post(body("email").isEmail().withMessage("Enter a valid email"), body("password").isLength({ min: 8, max: 16 }).withMessage("Password should be atleast 8 and maximum 16 characters"), async (req, res, next) => {
    ...
    try {
      let already = await Developer.findOne({ email })
      let alreadyPhone = await Developer.findOne({ phone })
      if (already) {
        return next(new ApiError(400, "Developer registration failed. Email Already Exist."))
      }
      if (alreadyPhone) {
        return next(new ApiError(400, "Developer registration failed. Phone Already Exist."))
      }
      else {
